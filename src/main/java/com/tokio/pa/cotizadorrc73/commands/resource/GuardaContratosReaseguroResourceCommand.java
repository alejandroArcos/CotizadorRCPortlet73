...
@Component(
		...
		property = {
			...
			"mvc.command.name=/cotizadores/reaseguro/guardaContratos"
		},
		service = MVCResourceCommand.class
	)

public class GuardaContratosReaseguroResourceCommand extends BaseMVCResourceCommand {
	...
	protected void doServeResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		String contratosReaseguroString = ParamUtil.getString(resourceRequest,
				"contratosReaseguro"); 
		...
		JsonArray contratosReaseguro = gson.fromJson(contratosReaseguroString, JsonArray.class);
		
		ReaseguroResponse responseMigracion = gson.fromJson(responseMigracionString,
				ReaseguroResponse.class);
		
		ReaseguroResponse response = _CMServicesReaseguro.guardarContratosReaseguro(cotizacion,
				version, folio, responseMigracion.getPoliza(), 
				responseMigracion.getCertificado(),
				responseMigracion.getFolio(), "", responseMigracion.getPaquete(),
				207, 1, 1, ramo, idPerfilUser, user.getScreenName(),
				CotizadorRCPortlet73PortletKeys.COTIZADORRCPORTLET73, contratosReaseguro);
